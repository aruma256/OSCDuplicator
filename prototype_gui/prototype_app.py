...
class PrototypeApp(UserControl):
    ...
    def __init__(self):
        ...
        self.transmitter_fields: list[TransmitterField] = self.__init_transmitter_fields()
    ...
    def transmitter_headline(self):
        return Row(
            controls=[
                Container(
                    content = Text(value="送信ポート"),
                    margin = 0,
                    width = 200
                ),
                Container(
                    content = Text(value="名前"),
                    margin = 0,
                    width = 300
                ),
                Container(
                    content = Text(value="enable"),
                    margin = 0,
                    width = 300
                )
            ]
    ...
    def __init_transmitter_fields(self):
        if self.duplicator.settings.transmit_port_settings:
            return [TransmitterField(self.duplicator, i) for i in range(len(self.duplicator.settings.transmit_port_settings))]
        else:
