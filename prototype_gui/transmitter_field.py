...
class TransmitterField(UserControl):
    ...
    def __init__(self, duplicator: Duplicator, index=0, empty=False):
        ...
        if empty:
            ...
            enabled = self.duplicator.settings.transmit_port_settings[index].enabled
    ...
    def build(self):
        return Row(
            [self.port, self.name, self.enabled]
        )
    ...
    def on_name_blur(self, e):
        self.duplicator.settings.transmit_port_settings[self.index].name = self.name.value
    ...
    def on_port_blur(self, e):
        try:
            ...
            self.duplicator.settings.transmit_port_settings[self.index].port = None
        else:
            self.port.counter_text = None
            self.duplicator.settings.transmit_port_settings[self.index].port = self.port.value

    def on_checkbox_change(self, e):
        self.duplicator.settings.transmit_port_settings[self.index].enabled = self.enabled.value
