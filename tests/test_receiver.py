...
from oscduplicator.receiver import OSCReceiver, OSCMessage

def test_osc_receiver_init():
    with mock.patch('oscduplicator.receiver.BlockingOSCUDPServer', autospec=True) as mock_server:
...
def test_start_receiver():
    with mock.patch('oscduplicator.receiver.BlockingOSCUDPServer', autospec=True) as mock_server, \
         mock.patch('oscduplicator.receiver.Thread', autospec=True) as mock_thread:
        receiver = OSCReceiver(5000)
        receiver.start_receiver()

        mock_thread.assert_called_once_with(target=mock_server.return_value.serve_forever)
...
def test_q_put():
    with mock.patch('oscduplicator.receiver.BlockingOSCUDPServer', autospec=True):
        receiver = OSCReceiver(5000)
        address = '/test'
        args = ['message']
